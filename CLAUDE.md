...
- `bun run test:deno` — run the suite under Deno (`deno test`). Deno type-checks each test module before running it, so a `tsc`-clean import can still fail here if it lacks an explicit extension.
  - The type-check resolves `node:` imports through `@types/node` in `node_modules`, so `bun install` must have run first (CI does this in the Deno job). Don't add a `deno.json` or commit a `deno.lock` — `bun.lock` is the single lockfile.
  - Deno recurses the whole repo for `*.test.ts`, so a scratch copy like `node_modules.bak/` gets picked up and fails the run — keep such copies outside the repo.
...
- `bun run lint` — [oxlint](https://oxc.rs) with type-aware rules (`oxlint-tsgolint`) over `src/`, configured in `.oxlintrc.json` (`correctness`/`suspicious`/`pedantic`/`perf`/`restriction` categories as errors — `style` is deliberately not enabled). The config is categories-only by design: no per-rule entries, no overrides. Keep it that way — when a rule fires, restructure the code rather than disabling or configuring the rule.
  ...
  - Probing a rule empirically: drop a `src/_probe.ts` with several candidate shapes, run `bun run lint`, then delete it — quicker than iterating on the real file.
...
- **Types** live in `src/hackjs.ts` (a types-only module, imported as `../hackjs.ts` — never a `.d.ts` file, which may not exist at runtime or after a build): `Bit = 0 | 1` and fixed-length readonly tuple types (`Bit2`…`Bit16`). Tuples are built by explicit construction (indexed element literals, rest-destructuring for address splits, `as const satisfies Bit16` for constant tables) — never by `as BitN` assertions on `slice`/`map` results, which the lint setup rejects as unsafe. Type-only imports must use `import type` — `tsconfig.json` enables `verbatimModuleSyntax`. The config extends only `@tsconfig/strictest` with `types: ["node"]`; no Bun-specific APIs or types are used, so the code stays portable across all three runtimes. `erasableSyntaxOnly` is on because Node and Deno run the `.ts` files by stripping types — enums, namespaces, parameter properties and `import x = require()` would pass `tsc` but crash at runtime, so `tsc` rejects them up front.
